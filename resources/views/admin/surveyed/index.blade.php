...
<script type="text/javascript">
    $(document).ready(function() {
        $('#confirmButton').click(function() {
            // Check if any specific field is empty
            if ($('#setup').val() === '') {
            ...
            if ($('#appro_h_f').val() === '') {
                alert('Please add approximate house of family.');
                return;
            }
            if ($('#appro_residents').val() === '') {
                alert('Please add approximate residents.');
                return;
            }
            if ($('#expected_cost').val() === '') {
                alert('Please add expected cost.');
                return;
            }
            if ($('#image').val() === '') {
            ...
            formData.append('setup', $('#setup').val());
            formData.append('project_id', $('#select_project').val());
            formData.append('status', $('#select_status').val());
            formData.append('area', $('#area').val());
            formData.append('village_name', $('#village_name').val());
            formData.append('region', $('#region').val());
            formData.append('appro_h_f', $('#appro_h_f').val());
            formData.append('appro_residents', $('#appro_residents').val());
            formData.append('expected_cost', $('#expected_cost').val());
            formData.append('remark', $('#remarks').val());

            // Get the file input element
            var files = $('#image')[0].files; // Access the files property to get the selected files
            for (var i = 0; i < files.length; i++) {
                formData.append('images[]', files[i]); // Append each file to FormData object
            }

            // Send an AJAX request to save the project data
            $.ajax({
                type: 'POST',
                url: '{{ route('save.surveyed') }}',
                data: formData,
                dataType: 'json',
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#errorContainer').html('<div class="alert alert-danger" role="alert">An error occurred: ' + xhr.responseText + '</div>');
    ...
    $(document).ready(function() {
        $('.view-record').click(function(e) {
            ...
            $.ajax({
                url: '/get-surveyed-details/' + recordId, // Replace with your route
                type: 'GET',
                success: function(response) {
                    // Clear previous data
                    $('#recordDetails').empty();
